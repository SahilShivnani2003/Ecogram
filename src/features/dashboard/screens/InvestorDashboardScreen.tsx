...
import { useGetMyBookings } from '@/features/bookings/hooks/useGetMyBookings';
...
type DashboardScreenProps = DrawerScreenProps<InvestorDrawerParamList, 'Home'>
...
const DashboardScreen = ({
    ...
    const investor = useAuthStore().user;
    // ── API data ──
    const { data: myBookings } = useGetMyBookings();
    const { data: myInvestments } = useGetMyInvestment();
    ...
    const totalEarned: number = walletData?.wallet?.totalEarned ?? 0;
    const rewardPoints: number = walletData?.wallet?.rewardPoints ?? 0;
    const activePlans: number = myInvestments?.summary?.activeCount ?? 0;
    const bookingsCount: number = myBookings?.bookings?.length ?? 0;
    const referralsCount: number = referralData?.stats?.total ?? 0;
    ...
    const stats: StatCardProps[] = [
        ...
        },
        {
            ...
            onPress: () => navigation.navigate('Refrels'),
            delay: 400,
        },
    ];
    ...
    const quickActions = [
        ...
        },
        {
            ...
            onPress: ()=> navigation.navigate('DailyIncome')
        },
        {
    ...
    const benefits = [
        ...
        {
            icon: '👥',
            title: '1% monthly referral income',
            subtitle: 'On referred active accounts',
        },
        {
            icon: '🎁',
            title: 'Reward points on every purchase',
            subtitle: 'Redeem for stays, products, cashback',
        },
                ...
                >
                    <View style={styles.welcomeLeft}>
                        ...
                        <View style={styles.welcomeText}>
                            <Text style={styles.welcomeTitle}>Welcome back, {investor?.name}!</Text>
                            <Text style={styles.welcomeEmail}>Investor · {investor?.email}</Text>
                        </View>
                    </View>
                    <TouchableOpacity
